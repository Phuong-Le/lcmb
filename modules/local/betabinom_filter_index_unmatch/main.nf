process betaBinomFilterIndexUnmatch {
    label 'process_tiny'
    ...
    tuple val(pdid), path("NR_bbinom_filtered.txt"), path("NV_bbinom_filtered.txt"), path("genotype_bin.txt"), optional: true, emit: phylogenetics_raw_input

    script:
    """
    Rscript --vanilla ${projectDir}/bin/beta_binom_filter_index_unmatch.R --libpath=${projectDir}/bin --cgpvaf_out=${vaf} --match_normal_id=${match_normal_id} --rho_threshold=${rho_threshold} --outdir=.
    """
}

process cgpVafConcat {
    label 'process_tiny'

    publishDir "${params.outdir}/filter_${mut_type}_out/${pdid}", mode: params.publish_dir_mode

    input:
    tuple val(pdid), val(sample_id_ls), val(match_normal_id), path("tmpvaf_${sample_id_ls[0]}_${chrom}"), val(chrom), path(vcf), path(vcf_tbi), path(bam), path(bai), path(bas), path(met), path(bam_match), path(bai_match)
    val mut_type
    path fasta
    path fai
